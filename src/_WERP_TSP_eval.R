...
## WEST WEIR ---------------------------------------------------------------
dss_out=opendss(paste0(data.path,"20230331/",alts[2],"/RSMGL_CEPP_output.dss"))
dss_cat=dss.cat.fun(dss_out,F)
dss_cat=subset(dss_cat,TYPE=="FLOW")
dss_cat[grepl("WEST",dss_cat$SITE),]
dss_cat[grepl("WEIR",dss_cat$SITE),]

RSM.sites=c("WESTWEIR")
rsm.Q=data.frame()
for(j in 1:n.alts){
  dss_out=opendss(paste0(data.path,"20230331/",alts[j],"/RSMGL_CEPP_output.dss"))  
...
rsm.Q$CY=as.numeric(format(rsm.Q$Date,"%Y"))
rsm.Q$month=as.numeric(format(rsm.Q$Date,"%m"))
rsm.Q$hydro.season=with(rsm.Q,ifelse(month%in%seq(6,10,1),"A_Wet","B_Dry"));# FL.Hydroseason(trans_flow.ol$Date)
rsm.Q$Alt=factor(rsm.Q$Alt,levels=alts)
rsm.Q$flow.kacft=cfs.to.acftd(rsm.Q$FLOW)/1000

subset(rsm.Q, flow.kacft<0)

range(rsm.Q$flow.kacft)

wweir.flow.seasonal=ddply(rsm.Q,c("Alt","CY","hydro.season"),summarise,TFlow.kacft=sum(ifelse(flow.kacft<0,0,flow.kacft),na.rm=T))
wweir.flow.seasonal=dcast(wweir.flow.seasonal,Alt~hydro.season,value.var="TFlow.kacft",mean)
# wweir.flow.seasonal=rbind(wweir.flow.seasonal,data.frame(Alt=alts[2],A_Wet=NA,B_Dry=NA))
# wweir.flow.seasonal=wweir.flow.seasonal[order(wweir.flow.seasonal$Alt),]
rowSums(wweir.flow.seasonal[,2:3])
rowSums(wweir.flow.seasonal[,2:3])[1]-rowSums(wweir.flow.seasonal[,2:3])[3]

wweir.flow.month=ddply(rsm.Q,c("Alt","CY","month"),summarise,TFlow.kacft=sum(ifelse(flow.kacft<0,0,flow.kacft),na.rm=T))
wweir.flow.month=ddply(wweir.flow.month,c("Alt","month"),summarise,mean.val=mean(TFlow.kacft,na.rm=T))
# wweir.flow.month=rbind(wweir.flow.month,
#                        data.frame(Alt=alts[2],month=1:12,mean.val=NA))
# wweir.flow.month=wweir.flow.month[order(wweir.flow.month$Alt),]

cols=c("lightblue","khaki3")
# png(filename=paste0(plot.path,"2023_AltNHF/WestWeir.png"),width=6.5,height=3.5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(2,2.5,0.5,1),oma=c(2,1,1,0.5),lwd=0.5);
...
tmp=t(wweir.flow.seasonal[,2:3])
x=barplot(tmp,col=cols,border="grey",
          space=0.05,
          ylim=ylim.val,axes=F,ann=F,names.arg = rep(NA,nrow(wweir.flow.seasonal)))
...
axis_fun(1,x,x,wweir.flow.seasonal$Alt,line=-0.5,cex=0.7)
...
mtext(side=2,line=2,"Discharge (x10\u2074 AcFt Y\u207B\u00B9)",cex=0.75)
mtext(side=1,line=1.5,"Alternatives",cex=0.75)
...
abline(h=0)
with(subset(wweir.flow.month,Alt==alts[1]),lines(month,mean.val,col=cols.alts[1],lty=1,lwd=1.5))
with(subset(wweir.flow.month,Alt==alts[2]),lines(month,mean.val,col=cols.alts[2],lty=2,lwd=1.5))
for(i in 3:n.alts){
  with(subset(wweir.flow.month,Alt==alts[i]),lines(month,mean.val,col=cols.alts[i],lty=1,lwd=1.5))
}
...
mtext(side=1,line=1.5,"Month",cex=0.75)
mtext(side=2,line=1.75,"Discharge (x10\u2074 AcFt M\u207B\u00B9)",cex=0.75)
...
plot(0:1,0:1,ylim=ylim.val,xlim=xlim.val,ann=F,axes=F,type="n")
abline(h=ymaj,v=xmaj,lty=3,col="grey")  
# with(ecdf_fun(subset(rsm.Q,Alt==alts[1]&flow.kacft>0)$flow.kacft),lines(1-proportion,value,col=cols.alts[1],lty=1,lwd=1.5))
with(ecdf_fun(subset(rsm.Q,Alt==alts[1])$flow.kacft),lines(1-proportion,value,col=cols.alts[1],lty=1,lwd=1.5))
with(ecdf_fun(subset(rsm.Q,Alt==alts[2])$flow.kacft),lines(1-proportion,value,col=cols.alts[2],lty=2,lwd=1.5))
for(i in 3:n.alts){
  with(ecdf_fun(subset(rsm.Q,Alt==alts[i])$flow.kacft),lines(1-proportion,value,col=adjustcolor(cols.alts[i],0.5),lwd=2))
}
axis_fun(1,xmaj,xmin,format(xmaj),line=-0.5)
axis_fun(2,ymaj,ymin,format(ymaj));box(lwd=1)
mtext(side=1,line=1.5,outer=F,"Proportion of Time \u2265 Discharge",cex=0.75)  
mtext(side=2,line=2,"Discharge Volume (x10\u2074 AcFt D\u207B\u00B9)",cex=0.75)
mtext(side=3,adj=1,"CY1965 - 2005")
dev.off()


## West boundary road culvert (to STOF reservation)
...
leg.fun(HP.bks,HP.pal,leg.type = "categorical",
        ...
        x.max=0.5,x.min=0.10)
dev.off()

  # tm_shape(HPmean)+tm_fill("ALTHNF",alpha=0.75,palette = HP.pal,breaks=HP.bks)+
  # tm_shape(rgeos::gIntersection(canals2,raster::aggregate(mesh1)))+tm_lines("blue",lwd=2)
...
# png(filename=paste0(plot.path,"2023_AltNHF/MeanHP_L28I_diff.png"),width=6.5,height=4,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(0.1,0.1,0.1,0.1),oma=c(0.5,0.5,1.5,0.5),lwd=0.5);
layout(matrix(c(1:3),1,3,byrow=T),widths=c(1,1,0.5))
# bbox.lims=bbox(bind(WG.ds.poly,LC.ds.poly))
bbox.lims=bbox(spTransform(gBuffer(spTransform(L28I.ds.poly,utm17),width=2000),NAD83HARN))
...
plot(0:1,0:1,ann=F,axes=F,type="n")
leg.fun(pal=pond.pal,leg.type = "categorical",
        leg.title="Annual Mean\nPonding Depth\n(Ft)",
        x.max=0.5,x.min=0.1,n.bks=6,
        labs=c("0","0 - 0.5","0.5 - 1.0", "1.0 - 2.0","2.0 - 3.0", "3.0 - 5.0"))
dev.off()
...
# png(filename=paste0(plot.path,"2023_AltNHF/MeanPondZ_L28I.png"),width=6.5,height=5,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(0.1,0.1,0.1,0.1),oma=c(0.5,0.5,1.5,0.5),lwd=0.5);
layout(matrix(c(1,2,5,3,4,6),2,3,byrow=T),widths=c(1,1,0.5))
# bbox.lims=bbox(bind(WG.ds.poly,LC.ds.poly))
bbox.lims=bbox(spTransform(gBuffer(spTransform(L28I.ds.poly,utm17),width=2000),NAD83HARN))
...
plot(0:1,0:1,ann=F,axes=F,type="n")
leg.fun(b=HP.bks,HP.diff.pal,leg.type = "categorical",
        leg.title="Difference\nin\nPonding Depth\n(Ft)",
        x.max=0.5,x.min=0.1)
dev.off()


# png(filename=paste0(plot.path,"2023_AltNHF/MeanPond_L28I_diff.png"),width=6.5,height=4,units="in",res=200,type="windows",bg="white")
par(family="serif",mar=c(0.1,0.1,0.1,0.1),oma=c(0.5,0.5,1.5,0.5),lwd=0.5);
layout(matrix(c(1:3),1,3,byrow=T),widths=c(1,1,0.5))
bbox.lims=bbox(spTransform(gBuffer(spTransform(L28I.ds.poly,utm17),width=2000),NAD83HARN))
...
WG.HP.mean=aggregate(HP~Alt,WG.ds.mean.vals,mean)
WG.HP.mean$HP[4]-WG.HP.mean$HP[1]
WG.Pond.mean=aggregate(PondDepth~Alt,WG.ds.mean.vals,mean)
WG.Pond.mean$PondDepth[4]-WG.Pond.mean$PondDepth[1]
