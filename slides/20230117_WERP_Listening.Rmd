...
bsplus::bs_button(
    ...
    button_size = "small") %>%
    htmltools::a(
      href = "https://swampthingecology.org/WERP_WQ/slides/20230117_WERP_Listening.pdf"
      )
```
]
...
.small[
.footnote[
<sup>1</sup> FDEP, 2017, Numeric Interpretation of Narrative Standards for the L-28 Interceptor Canal and Big Cypress National Preserve. Tallahassee, FL. [LINK to Document](https://swampthingecology.org/WERP_WQ/slides/FDEP WERP WQ WhitePaper_FINAL 071117.pdf)
]
]
...
Conceptual project placement - [map](#conmap)

---
name: Treat2_plugflow

### Conceptual WQ treatment projects 

#### Concept #1 - Filter marsh
.pull-left[
Simple Plug and Flow Equation
$$C_{O} = \frac{C_{i} - C^{*}}{(1+\frac{k}{Pq})^P}+C^{*}$$
.small[
$C_{O}$ = Outflow Conc (mg/L); $C_{i}$ = Inflow Conc (mg/L); 

$C^{*}$ = Background Conc (mg/L); k = areal rate coef (m/d);

P = TIS (assume 5 for this analysis);

q = HLR (m/d)
]

k-values within range of current STAs - [summary](#STA_k)

Average Q based on difference between WECB and ALTHR ([summary](#averageQ)) and a 50:50 split between Lardcan and Wingate mill canals



]
.pull-right[
```{r,out.width="100%",fig.align="center",fig.cap="Plug and Flow sensitivity analysis for Lardcan and Wingate Mill Filter marsh." }

knitr::include_graphics("./plots/WERP_FM_PnF.png")
```

* Assumes plug-and-flow conditions applicable to long-linear filter marsh. Some refinement may be needed.

* Some uncertainty in an appropriate k value to apply.
]
...
.pull-right[
```{r,out.width="40%",fig.align="center",fig.cap="Generalized diagram of 10-mile Canal filter marsh" }
...
*With more area comes more treatment* - [Example](#area_sens) of variability of outflow TP concentrations and k-value for Lardcan canal filter marsh. 

Outflow TP are dependent on [inflow volume](#vol_sens)(and k-value) at a given area.
...
Based on evaluation of past and ongoing pilot projects, the **cons** outweighs the pros for HWTT or chemical treatment and therefore it is <FONT COLOR="RED"> NOT </FONT> supported/recommended by SCCF and CoSWFL.
...
```{r,out.width="80%",fig.align="center",fig.cap="General conceptual project placement - NOT TO SCALE." }

knitr::include_graphics("./plots/GeneralConceptual_FilterMarshed.png")
```
...
```{r,out.width="80%",fig.align="center",fig.cap="Average annual discharge volume at WestWeir" }

knitr::include_graphics("./plots/AvgFloodControl.png")
```

[Return](#Treat2_plugflow)
---
name: area_sens
```{r,out.width="80%",fig.align="center",fig.cap="Range of area needed to treat inflow across various k rates for Lardcan canal filter marsh." }

knitr::include_graphics("./plots/WERP_FM_LC_areaPnF.png")
```

[Return](#Treat2_conclude)

---
name: vol_sens

```{r,out.width="80%",fig.align="center",fig.cap="Outflow TP concentration relative inflow volume across the various k rates for Lardcan canal filter marsh." }

knitr::include_graphics("./plots/WERP_FM_LC_Q_Co.png")
```
...
```{r,out.width="100%",fig.align="center",fig.cap="Calculated mean (± standard error) annual total phosphorus areal removal rates for Florida water year 2014 to 2021 (May 2013 – April 2021) observed in the Everglades Stormwater Treatment Areas and Northern Estuaries Protection Program Stormwater Treatment Areas. The horizontal solid line represents the mean areal removal rate across all STAs (12.2 m/yr)." }

knitr::include_graphics("./plots/PriorPresentations/STA_kvalues.png")
```
